...
import COS, { GetBucketResult } from "cos-js-sdk-v5";
import React, { useMemo, useState, MouseEvent, useEffect, MouseEventHandler } from "react";
...
export default function FileManager() {
  ...
  // 本次请求结果，用于判断分页
  const [result, setResult] = useState<GetBucketResult>({
    Name: "",
    Prefix: "",
    Marker: "",
    MaxKeys: "",
  ...
  /**单击面包蟹 */
  const changePrefix = (path: string) => {
    const newArr: string[] = [];
  ...
  /**获取COS API返回结果 */
  const getBucket = async (Marker?: string) => {
    setLoading(true);
  ...
  /**右键删除 */
  const deleteFile = (e: MouseEvent, Key: string) => {
    e.preventDefault();
  ...
  /**双击文件夹进入下一层 */
  const intoFolder = (Prefix: string) => {
...
/**面包屑元素 */
const BreadItem = ({ name, onClick }: BreadItemProps) => {
  return (
...
/**文件列表元素组件 */
const FileItem = ({ Icon, title, onDoubleClick, onContextMenu }: FileItemProps) => {
  return (
...
/**当前对象列表 */
type ListData = Pick<GetBucketResult, "CommonPrefixes" | "Contents">;
